...
import kotlinx.android.synthetic.main.fragment_kepsek_akun.view.*
import org.jetbrains.anko.sdk25.coroutines.onClick
import org.jetbrains.anko.support.v4.alert
import org.jetbrains.anko.support.v4.startActivity

class KepsekAkunFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val views = inflater.inflate(R.layout.fragment_kepsek_akun, container, false)

        firestore = FirebaseFirestore.getInstance()
        val spUser = this.activity?.getSharedPreferences("USERPREFS", Context.MODE_PRIVATE)
        views.button_kepsek_keluar.setOnClickListener {
        ...
        views.textView_kepsek_namalengkapE.text = arguments?.getString("nama")
        views.textView_kepsek_emailE.text = arguments?.getString("email")
        var aksesTeks = ""
        when(access) {
            "0" -> {
                aksesTeks = "Siswa "
        ...
        views.textView_kepsek_aksesE.text = aksesTeks

        views.textView_kepsek_layanan.onClick {
